...
/// <summary>
/// Generic query engine that applies filters, sorting, and pagination to IQueryable sources
...
internal sealed class QuerierEngine<T>
    where T : class
{
    private IQueryable<T>? _source;
    private QuerierParams? _queryParams;
    private Expression<Func<T, T>>? _fieldSelector;
    private IReadOnlyDictionary<string, Expression<Func<T, object>>>? _filterMappingFields;
    private IReadOnlyDictionary<string, LambdaExpression>? _sortMappingFields;
    private Func<string, bool>? _includeIsAllowed;
    private Func<IQueryable<T>, string?, IQueryable<T>>? _includeApplier;
    private Expression<Func<T,object>>? _fullTextSearchFieldExpression;
    ...
    public void SetQueryParams(QuerierParams queryParams)
    {
        _queryParams = queryParams;
    }

    public void SetFieldSelectors(Expression<Func<T, T>> fieldSelectors)
    {
        _fieldSelector = fieldSelectors;
    }
    public void SetFilterMappingFields(IReadOnlyDictionary<string, Expression<Func<T, object>>> filterMappingFields)
    {
        _filterMappingFields = filterMappingFields;
    }
    public void SetSortMappingFields(IReadOnlyDictionary<string, LambdaExpression> filterMappingFields)
    {
        _sortMappingFields = filterMappingFields;
    }
    ...
    public void SetFullTextSearchField(Expression<Func<T, object>> expression)
    {
        _fullTextSearchFieldExpression = expression;
    }
    ...
    private IQueryable<T> ApplyFullTextSearch(IQueryable<T> source, string? searchTerm)
    {
        ...
        if (_fullTextSearchFieldExpression is null)
            return source;

        var parameter = _fullTextSearchFieldExpression.Parameters[0];
        var body = UnwrapToActualType(_fullTextSearchFieldExpression.Body);
    ...
    {
        var normalizedPage = Math.Max(QuerierConstants.DefaultPageNumber, page);
        var normalizedPageSize = Math.Clamp(pageSize, QuerierConstants.MinPageSize, QuerierConstants.MaxPageSize);
        ...
        IReadOnlyDictionary<string, Dictionary<string, string>>? filters)
    {
        ...
        if (_filterMappingFields is null)
        {
            return source;
        }
        ...
        {
            if (!_filterMappingFields.TryGetValue(fieldName, out var memberExpression))
                continue;
    ...
    private IQueryable<T> ApplySort(IQueryable<T> source, string? sortQuery)
    {
        ...
        for (var i = 0; i < sortFields.Count; i++)
        {
            ...
            if (!_sortMappingFields.TryGetValue(fieldName, out var sortExpression))
                continue;
